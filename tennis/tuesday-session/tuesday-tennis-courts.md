# Tennis Court Arrangement Template

Using the template below, arrange Tuesday Tennis courts based on the RSVP list. Follow these rules when building the courts:

---

## PHASE 1: PLAYER LIST

**Step 1.1 - Compile the full player list**
Gather all players from:
- The WhatsApp event Going list (with levels from comments)
- Any players who posted directly in the group chat after the event was created

For each player note:
- Name
- Level: Beginner / Improver / Intermediate / Advanced
- Playtomic rating if known (0–7 scale, for reference)

**Step 1.2 - Count players per level**
```
🟢 Beginner:    X players
🟡 Improver:    X players
🟠 Intermediate: X players
🔴 Advanced:    X players

Total: X players → X courts needed
```

---

## PHASE 2: COURT ASSIGNMENT

**Step 2.1 - Calculate courts needed**
- Each court = 4 players
- If a level has 1–3 players, fold them into the nearest level
- Mark spare places if a court has fewer than 4

**Step 2.2 - Assign players to courts by level**
- Keep players grouped by level where possible
- Try to balance mixed gender within courts

**Step 2.3 - Court numbers (Up Cornellà)**
- Clay courts are called Tierra Batida (Batida 1–12)
- Book adjacent courts where possible so groups are near each other
- e.g. Beginners on Batida 6, Improvers on Batida 8, Intermediate on Batida 10, Advanced on Batida 12

---

## PHASE 3: BOOKER ASSIGNMENT

- The first player listed on each court is the **booker**
- Each player can only be booker **once** (if someone appears in multiple courts, assign a different booker for the second)
- The lead organiser (Jeffrey or Nadia) books court 1 and coordinates with other bookers

**Booker checklist:**
- [ ] Every court has exactly one booker
- [ ] No player is booker more than once
- [ ] Bookers are contacted directly and know their court number and time

---

## PHASE 4: VERIFICATION

- [ ] Every RSVP'd player appears in the output
- [ ] Each player is at the correct level
- [ ] No player appears twice in the same session
- [ ] Spare places are marked
- [ ] All bookers are unique

---

## PHASE 5: OUTPUT FORMAT

**WhatsApp formatting rules:**
- Bold: *text* (asterisks)
- Italic: _text_ (underscores)
- Bullet points: • (not - or *)
- Separators: --- on its own line
- No markdown headers - use bold text instead
- No tables

---

## TEMPLATE

```
🎾 *Tuesday Tennis - [DAY] [MONTH] [YEAR]* 🎾

---

⚠️ *CHECK YOUR VENUE & TIME* ⚠️

📍 *Venue:* Barcelona Pro Tennis Academy (Up Cornellà)
https://playtomic.io/tenant/9ce272f4-5039-4b6a-8f07-0c206712d06a

🗺️ *Address:* Cornellà-Riera (L8/S8/R5 from Plaça Espanya, ~40 min)
https://maps.app.goo.gl/nHkQHChjyc8RkyAH8

⏰ *Court time:* [19:30 or 20:30] (90 min)
🚉 *Meet at Plaça Espanya:* [TIME ~45 min before]

---

📱 *Instructions*

⚠️ BOOKERS: PLEASE BOOK YOUR COURT AS SOON AS POSSIBLE - CLAY SLOTS SELL OUT FAST ⚠️

📋 *For Bookers*
1. Open Playtomic and search *Barcelona Pro Tennis Academy*
2. Select Tuesday [DATE] and book your assigned Batida court at [TIME] (90 min clay)
3. Tap *Share Externally* and post the link in this group
4. Tag your 3 players in the same message so they can join and pay

🎯 *For Everyone*
1. Once your booker posts the link, join and pay your share promptly
2. If you need to drop out, find a replacement and let the group know early
3. If you can't find a replacement within 24h of the game, you may still owe your share

🤝 *On the day*
1. Meet at Plaça Espanya ~45 min before court time
2. There is a map/layout near the entrance at the venue - check your court number in the Playtomic app
3. Bring tennis balls (not padel balls!) and a racket if you have one

---

🎾 *Court Arrangements*

---

🟢 *BEGINNER*

*Court [X] - Batida [X] - 🟢 Beginner*
• [Name] 📋 _Booker_
• [Name]
• [Name]
• [Name]

---

🟡 *IMPROVER*

*Court [X] - Batida [X] - 🟡 Improver*
• [Name] 📋 _Booker_
• [Name]
• [Name]
• [Name]

---

🟠 *INTERMEDIATE*

*Court [X] - Batida [X] - 🟠 Intermediate*
• [Name] 📋 _Booker_
• [Name]
• [Name]
• [Name]

---

🔴 *ADVANCED*

*Court [X] - Batida [X] - 🔴 Advanced*
• [Name] 📋 _Booker_
• [Name]
• [Name]
• [Name]

---

🏅 *Stand-by:* [Name], [Name]
🃏 *Joker:* [Name] (available to fill any gap on the day)

---

_See you on the courts! 🎾_
```

---

## NOTES FOR ORGANISERS

- Post this message on **Wednesday** - the sooner it goes out, the less risk of losing clay court slots
- If a level only has 1–3 players, fold them into the nearest level group and note it
- When fewer than 4 courts are needed, skip the level sections with no players (don't include empty sections)
- If rain is forecast mid-week, flag it early and consider switching bookers to quick courts (cheaper, ~€7.80 pp vs €10 pp on clay)
- Always confirm with each booker directly (DM) so they don't miss the message in the group chat
